...
- **`prd.json` is not mutated mid-run beyond `passes`.** The PRD is author-time content; the harness only flips the `passes` flag on a story when the verifier signs off. Janitor/maintenance state lives in `progress.json`'s `timeline` array (a JSON ledger of `StoryEntry` and `JanitorEntry` rows). Anything that needs to record cross-iteration state goes there, not in `prd.json`.
