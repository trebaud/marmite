...
- **All runtime artifacts live under `.marmite/`.** Everything in `.marmite/` is tracked in git so the team shares the full project history (prd, progress, current-task, events, feedback). Never write marmite files to the project root, and never gitignore anything inside `.marmite/`.
